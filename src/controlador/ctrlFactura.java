...
public class ctrlFactura {
    ...
    public static factura crearFactura(String nombreUsuario, plan planUsuario, String archivoPDF) {
    if (contador >= MAX) return null;

    LocalDate hoy = LocalDate.now();
    String fecha = hoy.format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));

    factura f = new factura(IdFactura++, nombreUsuario, planUsuario.getIdPlan(), fecha, planUsuario.getValorMensual(), archivoPDF);
    facturas[contador++] = f;
    return f;
}
    ...
    public static boolean generarFacturaPDF(factura f, plan p) {
    try {
        ...
        doc.add(new Paragraph("Valor mensual: $" + p.getValorMensual()));
        doc.add(new Paragraph("\n"));
